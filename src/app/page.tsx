        ...
        <div className="relative max-w-7xl mx-auto px-8 py-24">
          <div className="text-center">
            ...
            {stats && !isLoading && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto mb-12">
                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
                  <div className="text-4xl font-bold text-white mb-2">{stats.totalEvents?.toLocaleString() || 0}</div>
                  <div className="text-sm text-gray-300">Events</div>
                </div>
                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
                  <div className="text-4xl font-bold text-white mb-2">{stats.totalEntities?.toLocaleString() || 0}</div>
                  <div className="text-sm text-gray-300">Entities</div>
                </div>
                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
                  <div className="text-4xl font-bold text-white mb-2">{stats.evolutionLinks?.toLocaleString() || 0}</div>
                  <div className="text-sm text-gray-300">Evolution Links</div>
                </div>
                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
                  <div className="text-4xl font-bold text-white mb-2">{stats.totalRelationships?.toLocaleString() || 0}</div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              <Link
                href="/graph"
                className="px-8 py-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl font-semibold hover:from-blue-600 hover:to-blue-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
              >
                Explore Graph →
              </Link>
              <Link
      ...
      <div className="max-w-7xl mx-auto px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-apple-lg p-8 border border-gray-100/50">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">Interactive Visualization</h3>
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-apple-lg p-8 border border-gray-100/50">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">Evolution Analysis</h3>
            <p className="text-gray-600">
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-apple-lg p-8 border border-gray-100/50">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">AllegroGraph RDF</h3>
            <p className="text-gray-600">
        ...
        {stats?.topEntities && stats.topEntities.length > 0 && (
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-apple-lg p-8 mb-8 border border-gray-100/50">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Top Entities by Connections</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {stats.topEntities.slice(0, 6).map((entity, idx) => (
                <div key={idx} className="flex justify-between items-center py-3 px-5 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl border border-gray-200/50 hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-600 bg-white px-3 py-1 rounded-lg">{entity.degree} connections</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-apple-lg p-8 border border-gray-100/50">
            ...
            <div className="space-y-3">
              {eventsData.data.slice(0, 5).map((event) => (
                <div key={event.eventId} className="p-5 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl hover:shadow-md transition-all duration-200 border border-gray-200/50">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <h4 className="font-semibold text-gray-900 mb-1">{event.label}</h4>
                      <p className="text-sm text-gray-600">{event.type}</p>
                    </div>
                    <span className="text-xs text-gray-500 bg-white px-3 py-1 rounded-lg">{event.date}</span>
                  </div>
                </div>
              ))}
            </div>
